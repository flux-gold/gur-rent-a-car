...
// GitHub Pages Uyumlu - TAMAMEN GÜNCELLENMİŞ
// ============================================
...
function initStorage() {
    ...
    // Varsayılan araç verileri (plaka son rakamları ile)
...
function updateVehicleSelect(select, vehicles) {
    ...
    availableVehicles.forEach(vehicle => {
        ...
        option.textContent = `${vehicle.name} (${vehicle.segment}) - ${vehicle.price} TL/gün - Plaka: ${vehicle.plateNumber || '70ABC' + vehicle.id}`;
...
function createVehicleCard(vehicle) {
    ...
    // Plaka numarasını göster
    const plateNumber = vehicle.plateNumber || '70ABC' + vehicle.id;
    const plateLastDigit = plateNumber.slice(-3); // Son 3 karakter
    ...
    card.innerHTML = `
        ...
        <div class="vehicle-image">
            ...
            <div class="plate-number" style="position: absolute; bottom: 10px; right: 10px; background: rgba(240, 192, 55, 0.9); color: #0F0F0F; padding: 4px 8px; border-radius: 4px; font-weight: bold; font-size: 12px;">
                Plaka: ${plateLastDigit}
            </div>
        </div>
        <div class="vehicle-details">
            <h3>${vehicle.name} <span style="font-size: 12px; color: #999;">(${plateNumber})</span></h3>
...
function initTestimonialSlider() {
    ...
    // Yavaş otomatik kaydırma başlat (10 saniyede bir)
    startSlowAutoScroll();
...
function startSlowAutoScroll() {
    ...
    // Yavaş kaydırma (10 saniyede bir)
